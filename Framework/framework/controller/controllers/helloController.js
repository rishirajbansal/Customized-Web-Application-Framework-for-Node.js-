...
const BaseRequest = require(path.join(__dirname, '../../valueObjects/requests/baseRequest'));


const logger = (new AppLogger(path.basename(__filename))).getLogger();


class HelloController extends AbstractControllers {
    ...
    handleRequest(request, response, next){
        logger.debug("Inside HelloController -> handleRequest()");
        
        const inboundRequest = new BaseRequest();
        ...
        try{
        	...
        	if (GenericUtility.safeTrim(actionMethod) != genConsts.EMPTY_STRING){
            	logger.debug("|~| Hello Request |~| Request recieved for action method : " + actionMethod);
            	logger.debug("|~| Hello Request |~| Request recieved from view : " + viewName);
            	
            	this.loadVORequestObject(request, inboundRequest, actionMethod);
            	
            	const action = ActionFactory.getActionInstance(actionDirectory.ACTION_HELLO);
            	...
            	action.execute(inboundRequest, actionMethod).then(
        			...
        			returnData => {
        				if (returnData.isInternalForward){
        					...
        					ViewsHandler.dispatch(request, response, next, genConsts.EMPTY_STRING, null, false, controllerDir.CONTROLLER_HELLO);
        				}
        				else{
            				...
            				ViewsHandler.dispatch(request, response, next, returnData.viewName, viewResponseData, true, controllerDir.CONTROLLER_HELLO);
        				}
        			},
        			error => {
        				AbstractControllers.handleError(request, response, next, viewName, error, controllerDir.CONTROLLER_HELLO);
        			}
        			
    			).catch(function (error) {
    				error.message = "[HelloController.handleRequest() - action.execute() - Promise Catch Handler] Exception Occurred : " + error.message;
    				logger.error(error);
    				AbstractControllers.handleError(request, response, next, viewName, error, controllerDir.CONTROLLER_HELLO);
    			});
    			
    			logger.debug("|~| Hello Request |~| Request handler returned after forwarding request to Promise");
    ...
    loadVORequestObject(request, inboundRequest, actionMethod) {
    	...
    	try{
    		...
    		logger.error("Exception occurred during the loading of VO object for Hello Controller : " + error.message);
